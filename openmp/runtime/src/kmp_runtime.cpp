...
int __kmp_get_global_thread_id() {
  ...
  if (TCR_4(__kmp_gtid_mode) >= 3) {
   ...
   //mjc
   int kmp_gtid = (int)(size_t)pthread_getspecific(__nk_kmp_gtid_threadprivate_key);
   return kmp_gtid;
   // return __kmp_gtid;
  }
#endif
...
int __kmp_get_global_thread_id_reg() {
  ...
  if (!__kmp_init_serial) {
    ...
    //mjc
    gtid = (int)(size_t)pthread_getspecific(__nk_kmp_gtid_threadprivate_key);
    // gtid = __kmp_gtid;
  } else
#endif
      if (TCR_4(__kmp_gtid_mode) >= 2) {
...
#ifdef KMP_TDATA_GTID
  //mjc
   pthread_setspecific(__nk_kmp_gtid_threadprivate_key,
                                 (void *)(intptr_t)(gtid));
  //__kmp_gtid = gtid;
#endif
